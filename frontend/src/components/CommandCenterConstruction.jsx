...
import { NinjaAI, NinjaSuccess } from "./NinjaIllustrations";
...
export default function CommandCenterConstruction({ onComplete }) {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      ...
      if (
        phaseElapsed >= PHASES[currentPhase].duration &&
  ...
  return (
    ...
    >
      ...
      <canvas
        ref={canvasRef}
        style={{ position: "absolute", inset: 0, opacity: gridOpacity * 0.4 }}
      />
      ...
      {!showForm ? (
        ...
        >
          ...
          <p style={{ fontSize: 13, color: "#94a3b8", marginBottom: 24 }}>
            Help KAGE AI personalize your experience. Quick setup — takes 30
            seconds.
          </p>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                {["low", "medium", "high"].map((level) => (
                  <button
                    key={level}
                    onClick={() =>
                      setFormData((p) => ({ ...p, risk_tolerance: level }))
                    }
                    style={{
                      flex: 1,
                      padding: "10px",
