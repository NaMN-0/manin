    ...
    if (compact) {
        return (
            <div
                ...
                onMouseEnter={e => e.currentTarget.style.background = 'var(--ninja-surface-hover)'}
    ...
    return (
        <div className="glass-card" onClick={onClick} style={{
            ...
            <div style={{
                ...
                <div>
                    <h3 style={{ fontSize: 18, fontWeight: 800, fontFamily: 'var(--font-mono)' }}>
                        {stock.ticker}
                    </h3>
                ...
                <span className={`badge ${isPositive ? 'badge-green' : 'badge-red'}`} style={{ fontSize: 13, padding: '6px 12px' }}>
            ...
            {stock.verdict && (
                <div style={{
                    ...
                    padding: '8px 12px',
                    borderRadius: 'var(--radius-sm)',
                    background: stock.verdict === 'STRONG BUY'
                        ? 'rgba(16,185,129,0.1)'
                        : stock.verdict === 'WATCHLIST'
                            ? 'rgba(245,158,11,0.1)'
                            : 'var(--ninja-surface)',
                    color: stock.verdict === 'STRONG BUY'
                        ? 'var(--emerald-glow)'
                        : stock.verdict === 'WATCHLIST'
                            ? 'var(--amber-glow)'
                            : 'var(--text-muted)',
                    ...
                    {stock.verdict === 'STRONG BUY' ? <Rocket size={14} /> : stock.verdict === 'WATCHLIST' ? <Eye size={14} /> : <Hand size={14} />}
                    {stock.verdict}
                </div>
            )}
        </div>
