    ...
    return (
        ...
        >
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
                ...
                <div>
                    <h3 style={{ fontSize: 20, fontWeight: 800, color: 'white', margin: 0 }}>{style.title}</h3>
                    <span style={{ fontSize: 11, color: color, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.1em' }}>
                        {style.subtitle}
                    </span>
                </div>
            </div>

            <div style={{ width: 100, height: 100, margin: '0 auto 16px', opacity: 0.8 }}>
                <style.Illustration width={100} height={100} />
            </div>

            <p style={{ fontSize: 13, color: '#94a3b8', marginBottom: 16, lineHeight: 1.5 }}>{style.desc}</p>

            <ul style={{ listStyle: 'none', padding: 0, margin: 0, fontSize: 12 }}>
                {style.features.map((f, i) => (
                    <li key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4, color: '#cbd5e1' }}>
                        <div style={{ width: 4, height: 4, borderRadius: '50%', background: color }} /> {f}
                    </li>
                ))}
            </ul>
        </div>
...
export default function CombatStyleSelector({ currentStyle, onSelect, onOptIn, optedIn }) {
    ...
    return (
        <div style={{ padding: '0', margin: '0' }}>
            <h2 style={{
                fontSize: 36, fontWeight: 900, textAlign: 'left', marginBottom: 12,
                background: 'linear-gradient(135deg, #fff, #94a3b8)',
                WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent',
            }}>
                Select Your Combat Style
            </h2>
            <p style={{ textAlign: 'left', color: '#94a3b8', marginBottom: 48, fontSize: 16, maxWidth: 600 }}>
                Choose how KAGE AI adapts to your trading personality. Each style unlocks specific tools and risk parameters.
            </p>
            ...
            <div style={{
                ...
                padding: 16, background: 'rgba(15,23,42,0.5)', borderRadius: 12, marginBottom: 24,
                border: '1px solid rgba(148,163,184,0.1)',
            }}>
                <Bell size={16} color="#64748b" />
                <label style={{ fontSize: 13, color: '#94a3b8', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8 }}>
                    <input type="checkbox" checked={latestUpdatesOnly} onChange={e => setLatestUpdatesOnly(e.target.checked)}
            ...
            <div style={{ textAlign: 'center' }}>
                <button onClick={handleConfirm} disabled={!selected} style={{
                    padding: '14px 48px', fontSize: 16, fontWeight: 800, borderRadius: 12,
                    background: selected ? 'var(--primary)' : '#1e293b', color: selected ? '#fff' : '#475569',
                    border: 'none', cursor: selected ? 'pointer' : 'not-allowed',
                    transition: 'all 0.3s', boxShadow: selected ? '0 0 30px rgba(14,165,233,0.3)' : 'none',
                }}>
                    Confirm Style
                </button>
            </div>
